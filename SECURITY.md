...
- Persistent authority-key creation and loading currently require Linux file-permission semantics. Pinpoint fails closed on macOS and Windows because Node cannot verify restrictive ACLs through the same opened file descriptor. Use Linux or an external signing boundary for operator-rooted receipts; ephemeral session receipt keys remain portable.
